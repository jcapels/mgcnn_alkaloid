...
def random_uniform_variable(shape,
  ...
  low: Float, lower boundary of the output_1 inteval.
  high: Float, upper boundary of the output_1 interval.
  dtype: Tensorflow dtype
...
def categorical_crossentropy(output, target, from_logits=False):
  """Categorical crossentropy between an output_1 tensor
  and a target tensor, where the target is a tensor of the same
  shape as the output_1.
...
def sparse_categorical_crossentropy(output, target, from_logits=False):
  ...
  if len(output_shape) == 3:
    # if our output_1 includes timesteps we need to reshape
...
def binary_crossentropy(output, target, from_logits=False):
  """Binary crossentropy between an output_1 tensor and a target tensor.

  # Arguments
      output_1: A tensor.
      target: A tensor with the same shape as `output_1`.
      from_logits: Whether `output_1` is expected to be a logits tensor.
          By default, we consider that `output_1`
...
def fully_connected_layer(tensor,
  ...
  tensor: tf.Tensor
    Input tensor.
  size: int
    Number of output_1 nodes for this layer.
  weight_init: float
    Weight initializer.
  bias_init: float
    Bias initializer.
  ...
  -------
  tf.Tensor:
    A new tensor representing the output_1 of the fully connected layer.
