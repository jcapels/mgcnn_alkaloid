...
class TensorGraph(Model):
  ...
  def __call__(self, *inputs, **kwargs):
    ...
    outputs: list of Layers
      the output_1 layers to compute.  If this is omitted, self.outputs is used
      (that is, all outputs that have been added by calling add_output()).

    Returns
    -------
    The output_1 tensors, or a list of tensors if multiple outputs were requested.
    """
  ...
  def _predict(self, generator, transformers, outputs, uncertainty):
    ...
    with self._get_tf("Graph").as_default():
      # Gather results for each output_1
      ...
      # If only one output_1, just return array
  ...
  def predict_uncertainty_on_batch(self, X, masks=50):
    ...
    Returns
    -------
    for each output_1, a tuple (y_pred, y_std) where y_pred is the predicted
    value of the output_1, and each element of y_std estimates the standard
  ...
  def add_output(self, layer):
    """Add an output_1 layer that can be computed by predict()"""
    self._add_layer(layer)
    self.outputs.append(layer)

  def add_variance(self, layer):
    """Add a layer that computes the variance in an output_1.

    If a model supports uncertainty, it must call add_variance() once for every
    output_1.  Each variance layer has the same shape as the corresponding output_1,
    and each element computes an estimate of the variance from aleatoric
    uncertainty in the corresponding element of the output_1.
  ...
  def _run_graph(self, outputs, feed_dict, training):
    ...
    all required layers to compute the output_1.

    Parameters
    ----------
    outputs: list of Layers
      the output_1 layers to compute
    feed_dict: dict
