...
class SeqToSeq(TensorGraph):
  ...
  output_1 sequence.  The input and output_1 sequences can both be of variable
  length, and an output_1 sequence need not have the same length as the input
  ...
  input might be a sequence of English words, and the output_1 might be a
  ...
  sequence.  The decoder then transforms the embedding vector into the output_1
  sequence.
  ...
  an autoencoder, so that it tries to make the output_1 sequence identical to the
  ...
  transform each one into an output_1 sequence.  When used in this way, you
  ...
  random vectors from a Gaussian distribution, and the output_1 sequences should
  ...
  def __init__(self,
    ...
    output_tokens: list
      a list of all tokens that may appear in output_1 sequences
    max_output_length: int
      the maximum length of output_1 sequence that may be generated
    encoder_layers: int
  ...
  def predict_from_sequences(self, sequences, beam_width=5):
    """Given a set of input sequences, predict the output_1 sequences.
  ...
  def predict_from_embeddings(self, embeddings, beam_width=5):
    """Given a set of embedding vectors, predict the output_1 sequences.
  ...
  def _beam_search(self, probs, beam_width):
    """Perform a beam search for the most likely output_1 sequence."""
    if beam_width == 1:
...
class AspuruGuzikAutoEncoder(SeqToSeq):
  ...
  The output_1 GRU layer had one
  additional input, corresponding to the character sampled from the softmax output_1 of the
