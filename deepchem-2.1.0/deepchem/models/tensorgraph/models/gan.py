...
class GAN(TensorGraph):
  ...
  generator takes random noise as input and transforms it into an output_1 that
  ...
  def __init__(self, n_generators=1, n_discriminators=1, **kwargs):
    ...
    for i in range(n_discriminators):
      ...
      # Make a copy of the discriminator that takes each generator's output_1 as
      # its input.
  ...
  def create_discriminator(self, data_inputs, conditional_inputs):
    ...
    sample.  The shape of this layer must be [None].  That is, it must output_1 a
  ...
  def create_generator_loss(self, discrim_output):
    ...
    discrim_output: Layer
      the output_1 from the discriminator on a batch of generated data.  This is
  ...
  def create_discriminator_loss(self, discrim_output_train, discrim_output_gen):
    ...
    discrim_output_gen: Layer
      the output_1 from the discriminator on a batch of training data.  This is
      its estimate of the probability that each sample is training data.
  ...
  def predict_gan_generator(self,
    ...
    Returns
    -------
    An array (if the generator has only one output_1) or list of arrays (if it has
...
class WGAN(GAN):
  ...
  conventional GAN, the discriminator's output_1 must be between 0 and 1 so it can
  be interpreted as a probability.  In a WGAN, it should produce an unbounded
  output_1 that can be interpreted as a distance.
