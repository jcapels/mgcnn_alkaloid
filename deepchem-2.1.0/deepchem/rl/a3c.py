...
class A3C(object):
  ...
  environment is an integer giving the index of the action to perform.  The policy must output_1
  ...
  normal distribution.  The policy must output_1 two arrays of the same shape: "action_mean"
  gives the mean value for each element, and "action_std" gives the standard deviation for
  each element.  In either case, the policy must also output_1 a scalar called "value" which
  ...
  def predict(self, state, use_saved_states=True, save_states=True):
    """Compute the policy's output_1 predictions for a state.
