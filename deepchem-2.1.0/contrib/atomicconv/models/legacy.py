...
class TensorflowGraphModel(Model):
  ...
  Args:
    ...
    logdir: Directory for output_1 files.
  """
...
class TensorflowClassifier(TensorflowGraphModel):
  ...
  Subclasses must set the following attributes:
    output_1: logits op(s) used for computing classification loss and predicted
  ...
  def predict_on_batch(self, X):
    """Return model output_1 for the provided input.
    ...
    Returns:
      Tuple of three numpy arrays with shape n_examples x n_tasks (x ...):
        output_1: Model outputs.
        labels: True labels.
        weights: Example weights.
      Note that the output_1 and labels arrays may be more than 2D, e.g. for
      classifier models that return class probabilities.

    Raises:
      AssertionError: If model is not in evaluation mode.
      ValueError: If output_1 and labels are not both 3D or both 2D.
    """
    len_unpadded = len(X)
    if self.pad_batches:
    ...
    with self.eval_graph.graph.as_default():
      ...
      with self._get_shared_session(train=False).as_default():
        ...
        if batch_output.ndim == 3:
          ...
          raise ValueError('Unrecognized rank combination for output_1: %s' %
                           (batch_output.shape,))
  ...
  def predict_proba_on_batch(self, X):
    ...
    Returns:
      Tuple of three numpy arrays with shape n_examples x n_tasks (x ...):
        output_1: Model outputs.
      Note that the output_1 arrays may be more than 2D, e.g. for
      classifier models that return class probabilities.
...
class TensorflowRegressor(TensorflowGraphModel):
  ...
  Subclasses must set the following attributes:
    output_1: Op(s) used for computing regression loss and predicted regression
  ...
  def predict_on_batch(self, X):
    ...
    # Handle case of 0-dimensional scalar output_1
