...
field_smiles = 'smiles'
path_alkaloid_data = 'training_set_overlapping.csv'
...
dataset_test = dc_loader.featurize("overlapping_test_set.csv")
...
def accuracy_multi_molecules(prediction, yyy, th=0.5):
    ...
    yyy = np.delete(yyy, columns_to_remove, axis=1)
    prediction = np.delete(prediction, columns_to_remove, axis=0)
...
df_results.to_csv('results_overlapping.csv', index=False)